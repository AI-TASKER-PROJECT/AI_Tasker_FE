-- Seed extra verified Business and Expert demo data for AI recommendation testing.
...
SELECT
    8100 + n,
    format('expert.recommend%02s@aitasker.local', n),
    '$2a$10$ZoOaEjQxlTUW.Rd9m0EEpefjYq735RpolRJMsjb5xz/5Z0DJp2RaC',
    format('0912%06s', n),
    format('Recommendation Expert %02s', n),
    2,
    'Approved',
    TRUE,
...
SELECT
    8400 + n,
    format('business.recommend%02s@aitasker.local', n),
    '$2a$10$ZoOaEjQxlTUW.Rd9m0EEpefjYq735RpolRJMsjb5xz/5Z0DJp2RaC',
    format('0922%06s', n),
    format('Recommendation Business %02s', n),
    1,
    'Approved',
    TRUE,
    CURRENT_TIMESTAMP - (n || ' days')::INTERVAL,
...
SELECT
    ...
    8100 + n,
    format('079206%06s', n),
    format('https://portfolio.aitasker.local/recommendation/experts/%s', n),
    3 + (n % 8),
    'Approved',
    1,
    CURRENT_TIMESTAMP - (n || ' days')::INTERVAL,
...
SELECT
    ...
    8400 + n,
    format('039900%04s', n),
    format('Recommendation Company %02s', n),
    format('District %s, Ho Chi Minh City', (n % 12) + 1),
    format('https://storage.aitasker.local/licenses/recommend-company-%s.pdf', n),
    'Approved',
    1,
    CURRENT_TIMESTAMP - (n || ' days')::INTERVAL,
    CURRENT_TIMESTAMP
...
SELECT
    ...
    END,
    3 + (n % 8),
    format('AI recommendation testing certificate set %s; RAG, NLP, API integration, deployment.', n),
    format('Expert %s has delivered AI chatbot, RAG knowledge base, e-commerce CRM, order management, API integration, testing, and deployment projects for recommendation matching.', n),
    CURRENT_TIMESTAMP - (n || ' days')::INTERVAL,
    CURRENT_TIMESTAMP
FROM expert_seed
...
SELECT
    8600 + n,
    8500 + n,
    format('Recommendation test AI chatbot and RAG project %02s', n),
    ...
    jsonb_build_object(
        'sow', jsonb_build_object(
            'title', format('AI chatbot RAG customer support project %s', n),
...
SELECT
    job_id,
    format('Recommendation SOW %02s', n),
...
SELECT
    ...
    format('Seed proposal %s matches the recommendation test job %s with AI chatbot, RAG, API integration, testing, and deployment experience.', n, job_n),
    format('https://storage.aitasker.local/proposals/recommendation/%s.pdf', n),
    jsonb_build_array(
...
WITH seeded_accounts AS (
    SELECT 8100 + n AS account_id, 2 AS role_id, 'EXPERT' AS wallet_type
    FROM generate_series(1, 20) AS n
    UNION ALL
    SELECT 8400 + n AS account_id, 1 AS role_id, 'BUSINESS' AS wallet_type
    FROM generate_series(1, 10) AS n
)
