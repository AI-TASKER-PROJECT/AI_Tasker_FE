...
export function ManageJobPage() {
  ...
  const minimumTimelineWeeks = Math.max(
    milestones.length + 1,
    totalMilestoneWeeks + 1,
  );
  const timelineValid =
    contractTimelineWeeks > Math.max(milestones.length, totalMilestoneWeeks);
  ...
  const createContract = async () => {
    ...
    if (!timelineValid) {
      setContractError(
        `Timeline hợp đồng phải lớn hơn tổng thời gian milestone (${totalMilestoneWeeks} tuần). Vui lòng nhập ít nhất ${minimumTimelineWeeks} tuần.`,
      );
      return;
    }
    setContractError("");
              ...
              {proposals.map((proposal) => (
                <ProposalCard
                  ...
                  onContract={() => {
                    ...
                    const suggestedWeeks = Math.ceil(
                      Number(proposal.deliveryDays || 0) / 7,
                    );
                    setContractForm((value) => ({
                      ...value,
                      contractTitle: `Contract - ${job.title}`,
                      timelineWeeks: String(
                        Math.max(minimumTimelineWeeks, suggestedWeeks || 6),
                      ),
                    }));
                  }}
                />
      ...
      >
        <div className="grid gap-4">
          ...
          {!timelineValid && (
            <Notice
              tone="warning"
              title={`Timeline phải lớn hơn tổng thời gian milestone (${totalMilestoneWeeks} tuần). Tối thiểu ${minimumTimelineWeeks} tuần.`}
            />
          )}
