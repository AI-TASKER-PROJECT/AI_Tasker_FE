  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description={staffMode ? 'Staff/Admin tiếp nhận, demo testing, viết technical report và đề xuất xử lý.' : 'Doanh nghiệp/chuyên gia tạo dispute để khóa dòng tiền và yêu cầu can thiệp.'}
      />
      <Card className="p-4">
        <SearchInput value={query} onChange={setQuery} placeholder="Tìm dispute theo job, trạng thái..." />
      </Card>
          ...
          <Card key={dispute.disputeId} className="p-5">
            <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
              <div>
                ...
                <h3 className="mt-3 font-display text-lg font-extrabold text-ink">{dispute.title}</h3>
                <p className="mt-2 text-sm text-slate-500">{dispute.jobTitle}</p>
                <p className="mt-3 max-w-3xl text-sm leading-6 text-slate-600">{dispute.evidenceReport}</p>
                <div className="mt-4 flex flex-wrap gap-2">
                  <Badge tone="slate">Người tạo: {dispute.raisedBy}</Badge>
                  <Badge tone="mint">Staff: {dispute.staffName || 'Chưa gán'}</Badge>
                </div>
              </div>
              <LinkButton to={staffMode ? `/app/tickets/${dispute.disputeId}` : `/app/disputes/${dispute.disputeId}`} variant="secondary">
                Xử lý
              </LinkButton>
            </div>
          </Card>
...
export function DisputeDetailPage({ staffMode = false }: { staffMode?: boolean }) {
  ...
  const technicalReport = async () => {
    setDispute(await disputeApi.technicalReport(dispute.disputeId, report.reportContent, report.proposedAction));
    setReportOpen(false);
  };
  const resolve = async () => {
    setDispute(await disputeApi.resolve(dispute.disputeId, report.proposedAction));
  };
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-6 xl:grid-cols-[1fr_420px]">
        <Card className="p-6">
          <div className="flex flex-wrap gap-2">
            ...
            {dispute.milestoneId && <Badge tone="amber">Milestone #{dispute.milestoneId}</Badge>}
          </div>
          <SectionHeading title="Bằng chứng và báo cáo" description="EvidenceReport trong back-end dùng cho cả bằng chứng, kết quả demo và technical report." />
          ...
          <div className="mt-5 grid gap-4">
            <Field label="Kết quả demo testing">
              <Textarea value={testResult} onChange={(event) => setTestResult(event.target.value)} placeholder="Ghi nhận môi trường test, AC đạt/không đạt, lỗi tái hiện..." />
            </Field>
        ...
        <Card className="p-6">
          ...
          <div className="mt-5 rounded-3xl bg-gradient-to-br from-brand-50 to-indigo-50 p-5">
            ...
            <p className="mt-2 font-display text-xl font-black text-ink">{dispute.proposedAction || 'Chưa có'}</p>
          </div>
          <Notice tone="warning" title="Termination snapshot" className="mt-4">
            Giao diện có chỗ cho Force Payout / Refund / split ratio; back-end hiện mới lưu proposedAction dạng text.
          </Notice>
          <div className="mt-5 grid gap-3">
            <Badge tone="slate">Raised by: {dispute.raisedBy || 'N/A'}</Badge>
            <Badge tone="mint">Assigned: {dispute.staffName || `Staff #${dispute.assignedStaffId || 'N/A'}`}</Badge>
          </div>
        </Card>
      </div>

      <Modal open={assignOpen} onClose={() => setAssignOpen(false)} title="Assign dispute" footer={<><Button variant="secondary" onClick={() => setAssignOpen(false)}>Hủy</Button><Button onClick={assign}>Gán staff</Button></>}>
        <Field label="Staff ID">
          <Input value={staffId} onChange={(event) => setStaffId(event.target.value)} />
        </Field>
      </Modal>

      <Modal open={reportOpen} onClose={() => setReportOpen(false)} title="Technical report" description="Staff ghi báo cáo tiếng Việt có dấu và đề xuất tỷ lệ chia tiền." footer={<><Button variant="secondary" onClick={() => setReportOpen(false)}>Hủy</Button><Button onClick={technicalReport}>Gửi report</Button></>}>
        <div className="grid gap-4">
          <Field label="Nội dung báo cáo">
            <Textarea value={report.reportContent} onChange={(event) => setReport((value) => ({ ...value, reportContent: event.target.value }))} />
          </Field>
          <Field label="Proposed action">
            <Input value={report.proposedAction} onChange={(event) => setReport((value) => ({ ...value, proposedAction: event.target.value }))} />
          </Field>
        </div>
      </Modal>
      ...
      {!staffMode && (
        <Notice tone="info" title="Luồng người dùng">
          Business/Expert xem trạng thái dispute tại đây. Staff/Admin dùng cùng detail nhưng có thêm ngữ cảnh xử lý ticket.
        </Notice>
      )}
    </div>
  );
      ...
      <Card className="p-5">
        ...
        <div className="mt-5 grid gap-3">
          {list.map((item) => {
            ...
            const title = isBusiness ? (item as BusinessProfile).companyName : (item as ExpertProfile).fullName || `Expert #${(item as ExpertProfile).expertId}`;
            const status = isBusiness ? (item as BusinessProfile).kybStatus : (item as ExpertProfile).kycStatus;
            const id = isBusiness ? (item as BusinessProfile).businessId : (item as ExpertProfile).expertId;
            return (
              <div key={id} className="flex flex-col gap-4 rounded-3xl border border-slate-100 p-4 md:flex-row md:items-center md:justify-between">
                <div className="flex items-center gap-3">
                  <Avatar name={title} />
                  <div>
                    <p className="font-extrabold text-ink">{title}</p>
                    <p className="text-sm text-slate-500">{isBusiness ? (item as BusinessProfile).taxCode : (item as ExpertProfile).nationalId}</p>
                  </div>
                </div>
                <div className="flex flex-wrap items-center gap-2">
                  <StatusBadge status={status} />
                  <LinkButton to={`/app/verifications/${tab}/${id}`} variant="secondary" size="sm">
                    Chi tiết
                  </LinkButton>
                </div>
              </div>
          ...
          {list.length === 0 && <EmptyState title="Chưa có hồ sơ" description="Không có hồ sơ trong nhóm này." />}
        </div>
      </Card>
    </div>
  );
...
export function VerificationDetailPage() {
  ...
  const approve = async (statusValue: 'Approved' | 'Rejected') => {
    const updated = await profileApi.approve(isBusiness ? 'BUSINESS' : 'EXPERT', profileId, statusValue);
    setProfile(updated);
  };
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        actions={<LinkButton to="/app/verifications" variant="secondary">← Danh sách</LinkButton>}
      />
      <div className="grid gap-6 lg:grid-cols-[1fr_360px]">
        <Card className="p-6">
          ...
          <div className="mt-5 grid gap-4 md:grid-cols-2">
            {isBusiness ? (
              <>
                <Info label="Mã số thuế" value={(profile as BusinessProfile).taxCode} />
                <Info label="Địa chỉ" value={(profile as BusinessProfile).address || 'Chưa có'} />
                <Info label="License URL" value={(profile as BusinessProfile).businessLicenseUrl || 'Chưa có'} />
              </>
            ) : (
              <>
        ...
        <Card className="p-6">
          ...
          <Notice tone="info" title="Audit trail" className="mt-4">
            Back-end ghi audit log khi duyệt. UI quản trị audit nằm ở module Admin.
          </Notice>
        </Card>
      </div>
    </div>
...
function Info({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-2xl bg-slate-50 p-4">
      ...
      <p className="mt-2 break-words text-sm font-extrabold text-ink">{value}</p>
    </div>
  );
}
    ...
    <div className="space-y-6">
      ...
      <Card className="p-6">
        <form onSubmit={submit} className="grid gap-4">
          ...
          <Button type="submit"><Gavel className="h-4 w-4" /> Tạo dispute</Button>
        </form>
        {created && <Notice tone="success" title={`Đã tạo dispute #${created.disputeId}`} className="mt-4" />}
      </Card>
    </div>
  );
}
