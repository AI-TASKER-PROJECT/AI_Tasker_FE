...
function PackageCard({
  ...
  const gradient = packageGradients[pkg.packageCode] ?? "from-brand-500 to-indigo-600";
  ...
  return (
    ...
    >
      {isPremium && (
        <div className="bg-gradient-to-r from-amber-400 to-orange-500 py-1.5 text-center text-xs font-black uppercase tracking-widest text-white">
          ⭐ Khuyến nghị
        </div>
      )}
      ...
      <div className={cn("bg-gradient-to-br p-6 text-white", gradient)}>
        <div className="flex items-start justify-between">
          <div>
            ...
            <h3 className="mt-3 font-display text-xl font-black">{pkg.packageName}</h3>
          </div>
          <div className="text-right">
            <p className="font-display text-3xl font-black">{formatCurrency(pkg.price)}</p>
        ...
        <ul className="space-y-3">
          ...
          {pkg.recommendVisibility && (
            <li className="flex items-center gap-2.5 text-sm">
              ...
              <span className="font-bold text-amber-700">AI Expert Recommendation</span>
            </li>
          )}
        </ul>
...
function CreditPurchaseModal({
  ...
  const [result, setResult] = useState<PaymentActionResponse<UserQuota> | null>(null);
  const [notice, setNotice] = useState<{ tone: "info" | "success" | "warning" | "danger"; msg: string } | null>(null);

  const unitPrice = role === "BUSINESS" ? 100_000 : 50_000;
  const creditType = role === "BUSINESS" ? "Job-post credit" : "Proposal credit";
  ...
  const purchase = async () => {
    ...
    try {
      ...
      setResult(res);
      if (res.completed) {
        setNotice({ tone: "success", msg: `Mua thành công ${qty} ${creditType}!` });
        onSuccess();
        window.dispatchEvent(new Event("aitasker:reload-wallet"));
      } else if (res.needTopup) {
        setNotice({ tone: "warning", msg: `Số dư không đủ. Cần thêm ${formatCurrency(res.missingAmount ?? 0)}.` });
      }
    } catch (err) {
    ...
    <Modal
      ...
      footer={
        <>
          ...
          <Button onClick={purchase} loading={loading} disabled={loading || !!result?.completed}>
      ...
      <div className="grid gap-4">
        ...
        {wallet && (
          <div className="rounded-2xl bg-brand-50 p-4 ring-1 ring-brand-100">
            ...
            <p className="mt-1 text-xl font-black text-brand-700">{formatCurrency(wallet.availableBalance)}</p>
          </div>
        )}
        <Field label="Số lượng credits">
          <div className="flex items-center gap-3">
            <Button
              ...
              size="icon"
              type="button"
              onClick={() => setQuantity((v) => String(Math.max(1, (Number(v) || 1) - 1)))}
            >
              −
            </Button>
            <Input
        ...
        <div className="rounded-2xl bg-slate-50 p-4 text-center">
          ...
          <p className="mt-1 font-display text-2xl font-black text-ink">{formatCurrency(total)}</p>
        </div>
        {result?.needTopup && (
...
export function MembershipPage() {
  ...
  const [purchaseResult, setPurchaseResult] =
    useState<{ result: PaymentActionResponse<MembershipPurchase>; pkg: MembershipPackage } | null>(null);
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description="Nâng cấp tài khoản để nhận badge xác minh, credits đăng job, nộp proposal và tính năng AI độc quyền."
      />
        ...
        <Card className="p-4">
          <div className="flex flex-wrap items-center gap-4">
            <div className="flex items-center gap-3 rounded-2xl bg-brand-50 px-4 py-2.5 ring-1 ring-brand-100">
              <span className="text-xs font-bold text-brand-600">Số dư khả dụng:</span>
              <span className="font-display text-lg font-black text-brand-700">
                {formatCurrency(wallet.availableBalance)}
              </span>
            </div>
            <p className="text-sm text-slate-500">
              Thanh toán được khấu trừ trực tiếp từ số dư ví.
            </p>
          </div>
        </Card>
      )}
      ...
      {isExternalRole && (
        <Card className="p-6">
          <SectionHeading
            ...
            description="Credits không đi kèm gói thành viên. Mua lẻ theo nhu cầu thực tế."
          />
            ...
            <div className="rounded-2xl border border-slate-100 p-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-slate-500">
                    {role === "BUSINESS" ? "100,000 VND / credit" : "50,000 VND / credit"}
                  </p>
                </div>
              </div>
              <p className="mt-3 text-xs text-slate-400">
                {role === "BUSINESS"
                  ? "Dùng để đăng job mới. Mỗi job publish tiêu tốn 1 credit."
                  : "Dùng để nộp proposal. Mỗi proposal tiêu tốn 1 credit."}
              </p>
            </div>
            <div className="flex items-center justify-center rounded-2xl bg-gradient-to-br from-brand-50 to-indigo-50 p-4">
              <Button onClick={() => setCreditModalOpen(true)} className="w-full">
                <ShoppingCart className="h-4 w-4" />
      ...
      {purchaseResult && (
        <Modal
          open
          onClose={() => { setPurchaseResult(null); load(); }}
          title={purchaseResult.result.completed ? "Mua gói thành công!" : "Không thể mua gói"}
          footer={
            <div className="flex gap-2">
              ...
              <Button variant="secondary" onClick={() => { setPurchaseResult(null); load(); }}>
            ...
            {purchaseResult.result.completed ? (
                ...
                <div className="flex flex-col items-center gap-3 py-4 text-center">
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-sm text-slate-500">
                      Đã thanh toán {formatCurrency(purchaseResult.pkg.price)} từ ví.
                    </p>
                  </div>
                </div>
                <Notice
                  tone="success"
                  title="Badge xác minh đã được kích hoạt"
                >
              ...
              <>
                <Notice
                  ...
                  title={purchaseResult.result.message ?? "Không thể hoàn tất giao dịch"}
                >
                  {purchaseResult.result.needTopup && (
                    <>
                      Số dư hiện tại:{" "}
                      <strong>{formatCurrency(purchaseResult.result.currentBalance)}</strong>.
                      Cần thêm:{" "}
                      <strong>{formatCurrency(purchaseResult.result.missingAmount)}</strong>
                    </>
                  )}
                </Notice>
              </>
