  ...
  const handleGoogleLogin = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      try {
        ...
        if (!response.ok) {
           throw new Error(data.message || 'Xác thực Google với máy chủ thất bại.');
        }
  ...
  return (
    <div className="animate-fade-in">
      <div className="text-center mb-8">
        <h1 className="font-headline-lg text-headline-md text-on-surface mb-2">Welcome Back</h1>
        <p className="font-body-md text-body-md text-secondary">Sign in to your account to continue.</p>
      </div>
      ...
      <form onSubmit={handleLogin} className="space-y-4">
        ...
        <div className="flex items-center gap-2 pt-2">
          <input type="checkbox" id="remember" className="rounded border-outline-variant text-primary focus:ring-primary h-4 w-4" />
          <label htmlFor="remember" className="font-body-md text-body-md text-secondary cursor-pointer">
      ...
      <div className="mt-6 flex items-center justify-between">
        ...
        <span className="p-2 font-label-sm text-label-sm text-secondary bg-surface-container-lowest">OR</span>
        <hr className="w-full border-outline-variant" />
      </div>
      ...
      */}
      ...
      <p className="mt-8 text-center font-body-md text-body-md text-secondary">
        Don't have an account? <a href="/auth/register-business" className="text-primary font-bold hover:underline">Sign up</a>
      </p>
    </div>
  );
};
